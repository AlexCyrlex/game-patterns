HonorBound 
===============================================================================
* Play Links: https://apps.facebook.com/honorbound/
* Info Links: http://forum.juiceboxmobile.com/forum/juicebox-games/honorbound - http://honorbound-juicebox.wikia.com/wiki/Honorbound_by_Juicebox_Wiki
* Screenshots:
* Videos: https://www.youtube.com/user/Unscenic/videos
* Created by: JuiceBox Games
* Tech: Unity
* Dates:
  * Public Access: 
  * Started: 2015.04.18 17:31
  * Last Update:
  * Closed: still active 

Patterns 
===============================================================================
* Theme: Fantasy, Against Chaos
* RPG
*
* __Actions__
* Combat
  * New Screen
  * Team: 4
  * Turns: Team
  * Attack, Ability, Wait
  * Goal: Kill Leader
  * Manual, Auto
* Path Boardgame
  * Branching
  * Step: Until Tile
  * Tiles: Enemy, Chest, Coins, Exit
* Collect Units
* Capture Units
* Steal
* Loot Now Keep On Win
  * For: Chests, Captures
  * Keep: XP, Gold, Steal
* Research 
* Speedup
* Identify Item
  * As: Research Artifact
* Train
  * As: Fortify
* Consume Units
  * For: Fortify
* Merge Units
  * As: Evolve
  * Number: 2
  * Increase: Evolve Rank, Stats
* Evolve
  * Cost: Target *2 (Same Rank), Embers
  * Result: Increase Rank, Change Rarity (Some)
* Produce Resource
  * As: Mine, Ember Baron
  * For: Gold, Embers
  * Continuous, Limit
* Crafting?
*
* __Social__
* Async PVP
  * Target: Random, Choose 3
* Win Streak
  * 1: 250 gold
  * 3: Mega Battle Token
  * 6: 2* bronze rune
  * 9: Arena Energy Pack
* Guild
* Friends
  * Max: 35
* Friend Points
  * As: Favor
  * Get: 5 * active friends  / day
*
* __Progress__
* XP Level
* Level Scaling Enemies
  * For: Portal, Event
* Level Scaling Event Only
* Classes?
  * Shadow Knight: Abyss
  * Spirit Sentinel: Spirit
  * Sylvan Protector: Primal
  * Pyromancer: Terra
  * Celestial Sage: Lunar
  * Warrior
  * Knight
  * Archer
  * Rogue
  * Wizard
  * Cleric
* Skills
* Quests
* Quests For Units
  * As: Quests
* Achievements
* Reputation
  * As: Renown
  * For: Region
  * Names: Neutral, Friendly, Liked, Respected
* Collection
  * For: Heroes
  * Reward: Legendary Hero
*
* __Motivators__
* Daily Reward
  * Max: 7d
* Time Limited?
* Energy
  * For: PVE, PVP
  * Max: 15 + 2 * research, 3
  * Recover: 1/5m, 1/1h
*
* __Payment__
* Shop?
* VIP Points
  * Get: Quests, Buy
* Activate VIP
  * Cost: Diamonds
* Pay To Use?
*
* __World__
* Location
  * Region
    * Dungeon
  * Mine
  * Portal
* NPC? 
* Map?
* Travel?
*
* __Things__
* Currency
  * Common: Gold
  * Premium: Diamonds
* Resource
  * Embers: Upgrade
  * Essence: Train
  * Rune: Bronze, Silver, Elite: Summon
  * Summoning Emblems: Summon Commander 
* Equipment
  * Ring *2, Necklace, ???
* Inventory
  * As: Hero Library
  * For: Units
  * Size: 25
* Expand Inventory
  * Add: 10
  * Cost: Diamonds 100, 125, 150
* Consumables
  * Heal, Revive
* Rarity
  * Name: Common, Uncommon, Rare, Epic, Legendary
  * Color: Brown, Silver, Golden, Purple, Orange
* Unit Ranks
  * As: Evolve Rank
  * Stars: 1-4
  * Name: X. X+. X++. X+++
*
* __Implementation__
* Limited Control Tutorial Bugs
*
* __Tricks__
* Keep Incomplete
  * For: Mine
  * Benefit: XP, Gold, Steal without Energy
* Reroll Instance
  * For: Dungeon
* One Battle Then Exit

New Ideas
===============================================================================
* _Patterns that appear new_

Lists 
===============================================================================
* Items
* Servers
  * [S9] Cross Road (HOT) [S8] Fading Light (HOT)  [S7] Enemy Force (HOT) [S6] Space Scar  [S5] Vanishing Sense   [S4] Phase Shift [S3] Silent Run  [S2] Collapsing Land  [S1] False Peace
  * Play: S9

Stats
===============================================================================
E.g. number of downloads, etc. 
